...
class LeadCreate(BaseModel):
    ...
    nomor_hp: Optional[str] = Field(None, max_length=20, description="Phone number for duplicate prevention")
...
async def create_lead(lead: LeadCreate, background_tasks: BackgroundTasks, db: PrismaClient = Depends(get_db)):
    ...
    try:
        ...
        if lead_update.notes is not None:
            update_data['encryptedData'] = json.dumps({
                'business_name': lead_update.business_name or existing_lead.business_name,
...
async def get_lead_stats(
    ...
    try:
        ...
        return {
            ...
            'hot_leads': hot_leads,
            'leads_by_status': {item['status']: item['_count']['status'] for item in leads_by_status}
        }
