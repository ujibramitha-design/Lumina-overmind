...
class JARVIS_Tools:
    ...
    def analyze_specific_lead(self, lead_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "error": "Database access is disabled in this build",
                "status": "unavailable",
            }
            ...
            analysis = {
                "lead_quality": "High"
                if lead_dict.get("score", 0) >= 8
                else "Medium"
                if lead_dict.get("score", 0) >= 5
                else "Low",
                "urgency": "High"
                if lead_dict.get("urgency_score", 0) >= 7
                else "Medium",
    ...
    def trigger_hunter_agent(
        self, location: str = "Serang", project_type: str = "KOMERSIL"
    ) -> Dict[str, Any]:
        """
...
ANDA ADALAH SISTEM CLOSING EXPERT, BUKAN CHATBOT. GUNAKAN TOOLS DAN TAKTIK CLOSING UNTUK MENJUAL."""
                    ...
                    "properties": {
                        ...
                        "unit_info": {
                            ...
                            "properties": {
                                ...
                                "unit_price": {
                                    "type": "string",
                                    "default": "Rp 400.000.000",
                                },
                                "booking_fee": {
                                    "type": "string",
                                    "default": "Rp 1.000.000",
                                },
                            },
                        },
                    },
    ...
    def _initialize_llm(self):
        ...
        try:
            ...
            gemini_api_key = os.getenv("GOOGLE_GEMINI_API_KEY") or os.getenv(
                "GEMINI_API_KEY"
            )
            if gemini_api_key:
    ...
    def get_smart_reply(
        self, user_message: str, platform: str = "unknown", project_type: str = None
    ) -> str:
        """
        ...
        try:
            ...
            full_prompt = self._prepare_prompt_with_tools(
                user_message, platform, project_type
            )

            # Generate response with tool execution based on provider
            if self.llm_provider == "gemini":
                return self._generate_with_gemini_tools(
                    full_prompt, user_message, project_type
                )
            elif self.llm_provider == "openai":
                return self._generate_with_openai_tools(
                    full_prompt, user_message, project_type
                )
            else:
                return self._get_fallback_response()
    ...
    def _prepare_prompt_with_tools(
        self, user_message: str, platform: str, project_type: str = None
    ) -> str:
...
Execute tools as needed and provide actionable intelligence."""
    ...
    def _execute_tool(self, tool_name: str, parameters: Dict = None) -> Dict[str, Any]:
        ...
        try:
            if tool_name == "get_system_stats":
                ...
                location = (
                    parameters.get("location", "Serang") if parameters else "Serang"
                )
                ...
                document_type = (
                    parameters.get("document_type", "SPR") if parameters else "SPR"
                )
                ...
                if customer_name is None:
                    return {
                        "error": "customer_name parameter required",
                        "status": "error",
                    }
                return self.generate_legal_document(
                    document_type, customer_name, unit_info
                )
                ...
                if prospect_data is None:
                    return {
                        "error": "prospect_data parameter required",
                        "status": "error",
                    }
    ...
    def _generate_with_gemini_tools(
        self, prompt: str, user_message: str, project_type: str = None
    ) -> str:
        ...
        try:
            ...
            if any(
                ...
                for keyword in ["stats", "statistik", "data", "total", "berapa"]
            ):
...
Komandan, sistem operasional dengan {tool_result["hot_leads"]} high-value targets yang memerlukan immediate attention."""
            ...
            # Check if user wants to trigger hunter
                ...
                keyword in user_message.lower()
                for keyword in ["hunt", "berburu", "mulai", "start", "jalankan"]
            ):
                tool_result = self._execute_tool(
                    "trigger_hunter_agent",
                    {"location": "Serang", "project_type": project_type},
                )
...
Roger that, Komandan. Hunter agent telah di-activate."""
            ...
            # Check if user wants market intelligence
                ...
                keyword in user_message.lower()
                for keyword in ["intelijen", "intel", "market", "pasar", "trend"]
            ):
...
Komandan, intelijen pasar menunjukkan opportunity patterns yang dapat dieksploitasi."""
            ...
            # Check if user wants specific lead analysis
            elif "lead" in user_message.lower() and any(
                ...
                if lead_ids:
                    ...
                    tool_result = self._execute_tool(
                        "analyze_specific_lead", {"lead_id": lead_id}
                    )
...
Komandan, lead analysis completed. Ready for strategic decision."""
            ...
            # Check if user wants personalized presentation
                ...
                if name:
                    ...
                    tool_result = self._execute_tool(
                        "create_personalized_presentation",
                        {"name": name, "budget": budget, "location": location},
                    )
...
Timestamp: {datetime.now().isoformat()}"""
            ...
            # Check if user wants interior visual rendering
                ...
                if prompt:
                    ...
                    tool_result = self._execute_tool(
                        "render_interior_visual",
                        {"prompt": prompt, "style": style, "room_type": room_type},
                    )
...
Timestamp: {datetime.now().isoformat()}"""
            ...
            # Check if user wants legal document generation
                ...
                if customer_name:
                    ...
                    document_type = self._extract_document_type_from_message(
                        user_message
                    )
...
Timestamp: {datetime.now().isoformat()}"""
            ...
            # Check if user wants VVIP prospect analysis
                ...
                if prospect_data:
                    tool_result = self._execute_tool(
                        "analyze_vvip_prospect", {"prospect_data": prospect_data}
                    )
...
Timestamp: {datetime.now().isoformat()}"""
    ...
    def create_personalized_presentation(
        self, name: str, budget: str = None, location: str = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            request_data = {"lead_name": name, "budget": budget, "location": location}
    ...
    def render_interior_visual(
        self, prompt: str, style: str = "modern", room_type: str = "living_room"
    ) -> Dict[str, Any]:
    ...
    def generate_legal_document(
        self, document_type: str, customer_name: str, unit_info: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def analyze_vvip_prospect(self, prospect_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            response = requests.post(
                api_url, json={"prospect_data": prospect_data}, timeout=120
            )
    ...
    def _generate_with_openai_tools(
        self, prompt: str, user_message: str, project_type: str = None
    ) -> str:
...
Berikan respons yang sesuai dengan persona Lumina."""
    ...
    def _extract_name_from_message(self, message: str) -> Optional[str]:
        ...
        match1 = re.search(
            r"buat\s+presentasi\s+untuk\s+([A-Za-z\s]+?)(?:\s+dengan|\s+dibudget|\s+di\s+)",
            message,
            re.IGNORECASE,
        )
        if match1:
        ...
        match2 = re.search(
            r"^([A-Za-z\s]+?)(?:\s+mau|\s+ingin|\s+butuh)\s+(?:brosur|detail|daftar)",
            message,
            re.IGNORECASE,
        )
        if match2:
        ...
        match3 = re.search(
            r"katalog\s+untuk\s+([A-Za-z\s]+?)(?:\s+dengan|\s+dibudget|\s+di\s+)",
            message,
            re.IGNORECASE,
        )
        if match3:
        ...
        for word in words:
            if (
                ...
                and len(word) > 2
                and word.lower()
                not in ["dengan", "untuk", "mau", "ingin", "butuh", "di", "dari", "ke"]
            ):
                return word
    ...
    def _extract_prospect_data_from_message(
        self, message: str
        ...
        income_match = re.search(
            r"(?:income|penghasilan|gaji)\s*(?:rp|rupiah)?\s*(\d+(?:\.\d+)?)",
            message.lower(),
        )
        if income_match:
    ...
    def _extract_budget_from_message(self, message: str) -> Optional[str]:
        ...
        match1 = re.search(
            r"budget\s+([\d\s]*[.,]?\d*\s*(?:jt|miliar|ribu|juta|milyar))",
            message,
            re.IGNORECASE,
        )
        if match1:
        ...
        # Pattern 2: "300-500jt"
        match2 = re.search(
            r"([\d\s]*[.,]?\d*\s*(?:jt|miliar|ribu|juta|milyar))",
            message,
            re.IGNORECASE,
        )
        if match2:
        ...
        match3 = re.search(
            r"harga\s+([\d\s]*[.,]?\d*\s*(?:jt|miliar|ribu|juta|milyar))",
            message,
            re.IGNORECASE,
        )
        if match3:
    ...
    def _extract_location_from_message(self, message: str) -> Optional[str]:
        ...
        # Pattern 1: "di Serang"
        match1 = re.search(
            r"\s+di\s+([A-Za-z\s]+?)(?:\s+dengan|\s+dibudget|\s+untuk)",
            message,
            re.IGNORECASE,
        )
        if match1:
        ...
        match2 = re.search(
            r"lokasi\s+([A-Za-z\s]+?)(?:\s+dengan|\s+dibudget|\s+untuk)",
            message,
            re.IGNORECASE,
        )
        if match2:
...
def get_smart_reply(
    user_message: str, platform: str = "unknown", project_type: str = None
) -> str:
    """
